...
focus=$(dumpsys activity activities 2>/dev/null |
  awk '/ResumedActivity:/ {print $4}' | head -n 1 | cut -d/ -f1)
if [ -z "$focus" ]; then
  focus=$(dumpsys window 2>/dev/null |
    awk '/mCurrentFocus=Window/ {print $3}' | head -n 1 | cut -d/ -f1)
fi
...
class TrackingPointsTile(QFrame):
    ...
    def set_values(self, left_lip: Any, right_lip: Any, mouth_span: Any) -> None:
        ...
        span_text = (
            f"Mouth span: {span_value} px"
            if span_value not in ("", "--")
            else "Mouth span: --"
        )
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._append_log(
            "Shortcuts: 1 raw, 2 lip points, 3 face mesh, "
            "R restart UI stream, C clear log, Esc quit"
        )
