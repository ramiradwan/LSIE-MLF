#!/usr/bin/env python3
"""
spec_ref_check.py — Resolve, index, and validate spec references (v3 schema)
=============================================================================

Auto-generates a spec_ref index from the content.json payload embedded in tech-spec,
then scans the project for all spec_ref usage and reports which refs
resolve, which don't, and which content paths have no ref pointing at them.
...
Usage:
    ...
    python scripts/spec_ref_check.py --from-pdf docs/tech-spec-v3.0.pdf --extract > content.json
...
def extract_content_from_pdf(pdf_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError(f"Could not read embedded files from {pdf_path}")
    ...
    try:
        ...
        raise ValueError(f"Failed to parse {target_name} from PDF: {e}")
...
class _IndexEntry:
    ...
    def __init__(
        self, title: str, content_path: str,
        content_type: str = "", preview: str = "", explicit: bool = False,
    ) -> None:
        self.title = title
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "title": self.title, "content_path": self.content_path,
            "content_type": self.content_type, "preview": self.preview,
...
_ROOT_KEY_TO_SECTION: dict[str, str] = {
    "document_control": "0", "system_philosophy": "1",
    "data_flow_pipeline": "2", "codebase_architecture": "3",
    "core_modules": "4", "data_governance": "5",
    "interface_contracts": "6", "math_specifications": "7",
    "llm_prompt": "8", "docker_topology": "9",
    "dependency_matrix": "10", "variable_matrix": "11",
    "error_handling": "12", "audit_checklist": "13",
}

_SUBSECTION_CONVENTIONS: dict[str, list[tuple[str, str]]] = {
    "document_control": [
        ("purpose_prose", "0.1"), ("normative_terms", "0.2"),
        ("canonical_terms", "0.3"), ("version_history", "0.4"),
        ("amendments", "0.5"),
    ],
    "codebase_architecture": [
        ("directory_hierarchy", "3.1"), ("build_context_rules", "3.2"),
    ],
    "data_governance": [
        ("vault_parameters", "5.1"), ("data_classifications", "5.2"),
        ("anonymization_rule", "5.3"),
    ],
    "interface_contracts": [("schema_definition", "6.1")],
    "llm_prompt": [
        ("inference_parameters", "8.1"), ("output_schema", "8.2"),
        ("system_prompt", "8.3"),
    ],
    "docker_topology": [
        ("containers", "9.1"), ("volumes", "9.2"),
        ("gpu_allocation_prose", "9.3"), ("device_passthrough_prose", "9.4"),
        ("network_prose", "9.5"), ("startup_order", "9.6"),
    ],
    "dependency_matrix": [
        ("system_requirements", "10.1"), ("pinned_packages", "10.2"),
    ],
    "audit_checklist": [("items", "13.1")],
}

# Math topic_id -> uppercase letter. Content.json uses 7A/7B, not 7.au12.
...
def build_index(spec: dict[str, Any]) -> dict[str, _IndexEntry]:
    ...
    for key, section_num in _ROOT_KEY_TO_SECTION.items():
        ...
        index[section_num] = _IndexEntry(
            ...
            content_path=key, content_type="section", preview=_preview(section),
        )
        ...
        for field_name, ref in mappings:
            ...
            if content is not None:
                index[ref] = _IndexEntry(
                    ...
                    content_type=type(content).__name__, preview=_preview(content),
                )
...
def _index_math_section(spec: dict[str, Any], index: dict[str, _IndexEntry]) -> None:
    """Index math specs using the 7A/7B letter convention."""
    ...
    if topics and isinstance(topics, list):
        for ti, topic in enumerate(topics):
            ...
            index[topic_ref] = _IndexEntry(
                ...
                content_type="MathTopic", preview=topic_title,
            )
            ...
            for si, step in enumerate(topic.get("derivation_steps", [])):
                ...
                index[step_ref] = _IndexEntry(
                    ...
                    content_type="MathStep", preview=step.get("title", ""),
                )
            if topic.get("reference_implementation"):
                ...
                index[impl_ref] = _IndexEntry(
                    ...
                    content_type="CodeBlock", preview="(Python reference implementation)",
                )
        return
        ...
        for sub in mod.get("subsections", []):
            ...
            if sub_num:
                index[sub_num] = _IndexEntry(
                    ...
                    content_path=f"core_modules.modules[module_id={mid}].subsections[number={sub_num}]",
                    content_type="SubSection", preview=_preview(sub),
                )
        if mod.get("contract"):
    ...
    for stage in spec.get("data_flow_pipeline", {}).get("stages", []):
        ...
        if num is not None:
            ...
            index[f"2.{num}"] = _IndexEntry(
                ...
                content_type="PipelineStage", preview=transition[:80],
            )
        ...
        for ref_obj in explicit:
            ...
            if ref:
                index[ref] = _IndexEntry(
                    ...
                    content_path=f"{key}.{ref_obj.get('content_path', '')}",
                    content_type="SpecRef (explicit)",
                    preview=ref_obj.get("title", ""), explicit=True,
                )
...
#   4.A.1, 4.C.3       (dot-separated module subsections)
#   12.3.2, 10.2.14    (numeric multi-level)
#   9, 12              (bare section numbers)
SPEC_REF_PATTERN = re.compile(r"\u00a7(\d+[A-Z]?(?:\.[A-Za-z0-9_]+)*)")

SCAN_GLOBS = ["**/*.py", "**/*.md", "**/*.yml", "**/*.yaml", ".claude/**/*.md"]
SCAN_EXCLUDE_DIRS = {
    ".git", "node_modules", "__pycache__", ".workspace_index",
    "venv", ".venv", "data",
}
            ...
            for line_num, line in enumerate(text.splitlines(), start=1):
                for match in SPEC_REF_PATTERN.finditer(line):
                    findings.append({
                        ...
                        "line": line_num, "context": line.strip()[:120],
                    })
    return findings
...
def validate_refs(
    index: dict[str, _IndexEntry], findings: list[dict[str, Any]],
) -> dict[str, Any]:
    ...
    for finding in findings:
        ...
        if ref in index:
            ...
            if parent and parent in index:
                resolved.append({**finding, "target": index[parent].content_path, "resolution": "parent"})
            else:
    ...
    return {
        ...
        "stats": {
            "total_refs_found": len(findings), "unique_refs": len(used_refs),
            ...
            "unresolved": len(unresolved), "index_size": len(index),
...
def main() -> int:
    ...
    parser.add_argument("--from-pdf", type=Path, default=None, help="Extract content.json from this PDF.")
    parser.add_argument("--content", type=Path, default=None, help="Path to standalone content.json.")
    parser.add_argument("--repo", type=Path, default=Path("."), help="Repository root (default: cwd).")
    parser.add_argument("--index", action="store_true", help="Print the full spec_ref index.")
    parser.add_argument("--validate", action="store_true", help="Scan project and validate all refs.")
    parser.add_argument("--resolve", type=str, default=None, help="Resolve a single ref (e.g., '7A.4').")
    parser.add_argument("--json", action="store_true", help="Output in JSON format.")
    parser.add_argument("--extract", action="store_true", help="Extract content.json from PDF to stdout.")
    ...
    if args.resolve:
        ...
        if entry:
            if args.json:
                print(json.dumps({ref: entry.to_dict()}, indent=2))
            else:
            ...
            if prefix_m:
                ...
                if suggestions:
                    suggestion_list = ', '.join('\u00a7' + s for s in sorted(suggestions, key=_sort_ref)[:10])
                    print(f"  Did you mean: {suggestion_list}", file=sys.stderr)
            return 1

    if args.index:
        if args.json:
            print(json.dumps({r: e.to_dict() for r, e in index.items()}, indent=2))
        else:
            print(f"Spec Reference Index ({len(index)} entries)\n{'=' * 72}")
            for ref in sorted(index, key=_sort_ref):
                entry = index[ref]
                marker = " *" if entry.explicit else ""
                print(f"  \u00a7{ref:<20} \u2192 {entry.content_path}{marker}")
            explicit_count = sum(1 for e in index.values() if e.explicit)
            print(f"\n  {explicit_count} explicit (marked *), {len(index) - explicit_count} structural\n")
    ...
    if args.validate:
        ...
        if args.json:
            print(json.dumps({"stats": stats, "unresolved": report["unresolved"]}, indent=2))
        ...
        print(f"  Index size:               {stats['index_size']} ({stats['index_explicit']} explicit, {stats['index_structural']} structural)")
