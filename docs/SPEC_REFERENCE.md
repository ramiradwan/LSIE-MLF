...
This file extracts the key implementation-governing details from the currently committed, signed Master Technical Specification PDF (`docs/tech-spec-v*.pdf`) and its extracted machine-readable index. The current base text includes the physiology/co-modulation, operator-console, corrected Oura-ingestion, acoustic-analytics, attribution, and deterministic semantic-scorer rollouts. `docs/SPEC_AMENDMENTS.md` remains the traceability registry for historical amendments and any future accepted deviations not yet folded into the signed PDF. The repository retains the legacy/server physiology path where webhook notifications enqueue hydration work to `physio:hydrate`, the API-hosted Oura hydration worker fetches provider resources and emits PhysiologicalChunkEvent records to `physio:events`, the Orchestrator derives scalar `_physiological_context` snapshots from rolling chunk buffers, and Module E persists derived records for reward results and Thompson Sampling state, physiology/co-modulation, observational acoustics, bounded semantic outputs, and attribution ledger entries. The v4 Desktop Runtime launched with `python -m services.desktop_app` is Redis-free: local process transport uses IPC queues/shared-memory blocks, local analytics state uses SQLite WAL, and cloud synchronization uses the local outbox rather than a Message Broker. Operator-facing analytics and session-state reads are served through local desktop API surfaces or retained server/cloud API surfaces, depending on deployment, and are not part of the server/cloud Persistent Store persistence boundary. Claude Code loads this on demand via the `@docs/SPEC_REFERENCE.md` import in CLAUDE.md.

For the signed base spec, see the single `docs/tech-spec-v*.pdf` file committed in this repository; the repository's review-controlled machine-readable contract index is `docs/content.json`. `python scripts/spec_ref_check.py --extract > docs/content.json` regenerates the checked-in index from the signed PDF. Runtime validation tooling uses the single committed PDF as the default authoritative source for spec-reference resolution, while `docs/content.json` remains the review-controlled machine-readable contract surface consumed by schema and registry validation. The amendment registry defines the current governing behavior wherever an amendment overrides the base spec.
...
`python -m services.desktop_app` starts the Redis-free desktop process graph. Mobile USB capture is supervised by `capture_supervisor`, Module C assembly runs in `module_c_orchestrator`, 30-second PCM windows move through local IPC queues/shared-memory blocks to `gpu_ml_worker`, and analytics/state writes are owned by `analytics_state_worker` against SQLite WAL. Offline cloud synchronization drains a local SQLite outbox; it does not require Redis, Celery, Docker Compose, PostgreSQL, or the Message Broker to launch or process local segments.

### Retained legacy/server pipeline
...
The v4 Desktop Runtime is a Windows-native ProcessGraph launched with `python -m services.desktop_app`: `ui_api_shell`, `capture_supervisor`, `module_c_orchestrator`, `gpu_ml_worker`, `analytics_state_worker`, and `cloud_sync_worker`. Local transport is IPC queues/shared-memory blocks, local persistence is SQLite WAL, and Redis is not a launch or runtime prerequisite.

Retained legacy/server container topology remains explicitly scoped to server/cloud deployments: redis:7-alpine → postgres:16-alpine → stream_scrcpy(ubuntu:24.04+scrcpy) → worker(nvidia/cuda:12.2.2-cudnn8) + orchestrator(nvidia/cuda:12.2.2-cudnn8) → api(python:3.11-slim, depends on redis + worker + postgres for physiology ingress, task dispatch, and persistence). Network: appnetwork bridge. Shared volume: ipc-share at /tmp/ipc/.

## §4.E.1 — Operator Console (SPEC-AMEND-008)

The retained operator surface is the PySide6 Operator Console running on the
operator's host, not a container. In retained server/cloud deployments, data
flows through the API Server's `/api/v1/operator/*` aggregate routes — the
console does not open direct Postgres or Redis connections. The v4 desktop
runtime instead launches `python -m services.desktop_app`, hosts the local
loopback API in `ui_api_shell`, and reads/writes SQLite-backed local state
without Redis. Launch the retained standalone console with
...
### SPEC-AMEND-001: Worker GPU Architecture Downgrade — *historical in v4.0*

The v3.4 cuDNN 8 / Pascal (SM 6.1) posture documented in SPEC-AMEND-001 is historical in the current v4.0 signed spec. The governing v4.0 requirements now live in the signed PDF's system runtime requirements and pinned dependency matrix (§10.1–§10.2), the desktop runtime topology and startup sequence (§9.4–§9.5), and the `gpu_ml_worker` failure-handling requirements (§12.4): production requires NVIDIA Turing (SM 7.5+) on CUDA 12.x with cuDNN 9, paired with `ctranslate2 >= 4.5.0` and `torch == 2.4.x`, while Pascal developer hosts route via `LSIE_DEV_FORCE_CPU_SPEECH=1` for CPU speech with GPU semantic scoring preserved. `faster-whisper.compute_type` remains locked to `"int8"`. The historical SPEC-AMEND-001 entry in `docs/SPEC_AMENDMENTS.md` is retained for traceability.
