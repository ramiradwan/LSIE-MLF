...
**Why this file exists.** When the ADO agent (or any contributor) does feature work, it must not "fix" a deferred integration by wiring it in without first satisfying the gating dependency. Activating any of these without the gate would either (a) crash at runtime against a missing external service, (b) violate the signed v3.2 spec (`docs/tech-spec-v3.2.pdf`), or (c) silently change the semantics of a stable subsystem.
