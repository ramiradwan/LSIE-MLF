...
| Field | Value |
|---|---|
| **Files** | `packages/ml_core/attribution.py` (`build_attribution_ledger_records(..., finality=...)`, replay-stable UUID helpers, and score identity that excludes finality so finalization can upsert the same rows), `packages/schemas/attribution.py` (`AttributionEvent`, `OutcomeEvent`, `EventOutcomeLink`, `AttributionScore` allow `online_provisional` and `offline_final`), `services/worker/pipeline/analytics.py` (`MetricsStore.persist_attribution_ledger()` / `_write_attribution_ledger()` upsert all four attribution tables), `services/desktop_app/processes/analytics_state_worker.py` (desktop SQLite upserts for `AttributionEvent`, `OutcomeEvent`, `EventOutcomeLink`, and `AttributionScore`, plus optional cloud handoff of the `online_provisional` event), `services/desktop_app/processes/gpu_ml_worker.py` (desktop IPC pass-through when attribution keys are supplied), `data/sql/05-attribution.sql` (finality constraints and read-path indexes), `services/desktop_app/state/sqlite_schema.py` (desktop attribution tables and indexes) |
| **Gating dependency** | A runtime finalization/replay producer that selects attribution events whose horizons have closed, rebuilds the ledger with `finality="offline_final"`, and invokes the existing idempotent upsert path. No production job, desktop process, Celery task, CLI, API route, or scheduler currently calls the builder with `offline_final` or replays closed horizons. |
| **Deferred since** | 2026-04-29 (narrowed 2026-05-11 after desktop local `online_provisional` parity landed) |
| **Justification** | The shipped `online_provisional` path is now wired in both retained server/cloud and desktop-local flows: worker analytics persists the full ledger in PostgreSQL, while the desktop runtime can pass attribution inputs through existing IPC models, upsert all four attribution tables in SQLite, and emit the optional cloud `AttributionEvent` in order with the segment payload and posterior delta. The remaining missing surface is the closed-horizon `offline_final` replay driver that decides when horizons are closed and feeds finalized records back through the same upsert path. Wiring finalization without that horizon scan and replay contract would either mark records final too early or leave analytics consumers unable to distinguish provisional data from closed-horizon attribution. |

### 6. Desktop Cloud Sync Expansion Beyond Current Producers
...
| **Gating dependency** | Remaining desktop cloud sync expansion beyond the current telemetry/upload surfaces. The active runtime now enqueues `InferenceHandoffPayload`, online-provisional `AttributionEvent`, and `PosteriorDelta` from `analytics_state_worker`, and the Operator Console Health page now exposes interactive cloud sign-in plus verified experiment bundle refresh. Still-deferred work is any future expansion beyond those shipped surfaces, such as new producer types or broader cloud-control lifecycles not yet defined by the signed spec. |
| **Deferred since** | 2026-05-02 (WS5 P2 desktop cloud-sync/offline-cache cycle; narrowed 2026-05-10 after WS5 P4 producer/auth wiring landed) |
| **Justification** | `cloud_sync_worker` is no longer waiting on desktop telemetry producers or operator-triggered auth surfaces: the outbox now receives typed segment telemetry, online-provisional attribution events, and posterior deltas from `analytics_state_worker`, and operators can launch PKCE sign-in and verified bundle refresh from the Health page. What remains deferred is not the shipped upload path itself but any future desktop cloud-sync expansion beyond the currently wired segment/attribution/delta producers and Health-page auth/bundle actions. |
...
- **Attribution online-provisional ledger writes** — the retained streaming path (`services/worker/tasks/inference.py` → `packages/ml_core/attribution.build_attribution_ledger_records()` → `services/worker/pipeline/analytics.MetricsStore.persist_attribution_ledger()`) is wired for `online_provisional` records, and the desktop runtime now mirrors that local persistence path in `services/desktop_app/processes/analytics_state_worker.py` when attribution inputs are supplied through the existing IPC contract. Only the closed-horizon `offline_final` replay/finalization path is deferred in entry #5.
