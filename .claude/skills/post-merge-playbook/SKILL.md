...
1. **Canonical Terminology Sweep** — run the §0.3 retired-synonym grep from `CLAUDE.md` and the longer Phase 6 list against `services/`, `packages/`, and `docker-compose.yml`. Zero matches in code/docstrings is the gate. README narrative prose is exempt.
