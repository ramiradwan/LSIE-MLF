...
2. **Documentation Reconciliation** — diff merge against `README.md`, `.claude/skills/*/SKILL.md`, `CLAUDE.md`, `docs/SPEC_REFERENCE.md`, and `docs/SPEC_AMENDMENTS.md`. Every new desktop process, env var, port, schema field, local SQLite table, IPC channel, or Redis key in code must surface in at least one doc. Container manifests are expected only if a future spec change reintroduces them. New spec deviations → register a SPEC-AMEND entry.
