...
Historical plans from prior cycles may be archived under `docs/artifacts/` when they remain useful as durable reference material.
...
Both commands assume that **every implementation decision traces to a spec section** — the phases you write here must cite the §-references that govern each file. The signed spec is the single `docs/tech-spec-v*.pdf` match; `scripts/spec_ref_check.py` extracts and validates its embedded content payload.
...
- **Spec updates** — governed by the signed PDF/content payload, not this implementation scaffold.
